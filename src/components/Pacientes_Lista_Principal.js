...
import { Search_Bar_Principal } from "./Search_Bar_Principal";
...
export default function Pacientes_Lista() {
  ...
  const {searchedPacientes,paciente, cargar_pacientes ,lista_pacientes,setlistapacientes,eliminar_paciente,limpiar_paciente} = useContext(pacientecontext); 
  ...
  return (
    <>
      <Grid sx={{ marginTop: "6rem", marginBottom: "1rem" }}>
        <Search_Bar_Principal />
        <Typography variant="h5">Lista Pacientes</Typography>
      </Grid>
      {searchedPacientes.map((paciente) => (
        <Box
